  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl p-8 max-w-lg w-full animate-fade-in-up max-h-[90vh] overflow-y-auto">
        <h3 className="text-2xl font-serif font-semibold text-[#162733] mb-6">{wall ? 'Edit' : 'Add'} Wall Painting</h3>
        <div className="space-y-4">
          ...
          <label className="flex items-center">
            <input type="checkbox" name="hasStairway" checked={formData.hasStairway} onChange={handleChange} className="h-4 w-4 rounded border-2 border-gray-400 text-[#093373] focus:ring-[#093373] mr-2" />
            Has Stairway
          </label>
          {formData.hasStairway && (
            <div>
              <label htmlFor="stairwaySqFt" className="block text-sm font-medium text-gray-700">Stairway SqFt</label>
              <input type="number" min="0" step="0.1" id="stairwaySqFt" name="stairwaySqFt" value={formData.stairwaySqFt || ''} onChange={handleChange} className={`mt-1 block w-full rounded-md shadow-sm border-2 border-gray-400 focus:ring-[#093373] text-gray-900 ${fieldErrors.stairwaySqFt ? 'border-red-500' : 'focus:border-[#093373]'}`} />
              {fieldErrors.stairwaySqFt && <p className="text-red-500 text-sm">{fieldErrors.stairwaySqFt}</p>}
              <label className="flex items-center">
                <input type="checkbox" name="hasRisers" checked={formData.hasRisers} onChange={handleChange} className="h-4 w-4 rounded border-2 border-gray-400 text-[#093373] focus:ring-[#093373] mr-2" />
                Has Risers
              </label>
              <label className="flex items-center">
                <input type="checkbox" name="hasRailings" checked={formData.hasRailings} onChange={handleChange} className="h-4 w-4 rounded border-2 border-gray-400 text-[#093373] focus:ring-[#093373] mr-2" />
                Has Railings/Spindles
              </label>
            </div>
          )}
          <label className="flex items-center">
